...
class IntentHandler : AbstractIntentHandler() {
    ...
    private fun handleReviewIntent(
        ...
        val deckId =
            if (intent.getBooleanExtra(EXTRA_SHOW_FIRST_DECK_CARD, false) ||
                reviewerIntent.getBooleanExtra(EXTRA_SHOW_FIRST_DECK_CARD, false)
            ) {
                CollectionManager.getColUnsafe().firstUserDeckId() ?: 0L
            } else {
                intent.getLongExtra(EXTRA_DECK_ID, 0)
            }
        if (deckId == 0L) {
            Timber.w("No deck to review (first deck missing or id 0)")
            showThemedToast(this, getString(R.string.first_deck_not_found), shortLength = true)
            launchDeckPickerIfNoOtherTasks(reloadIntent)
            return
        }
    ...
    companion object {
        ...
        const val EXTRA_SHOW_FIRST_DECK_CARD = "EXTRA_SHOW_FIRST_DECK_CARD"
        ...
        fun getLaunchType(intent: Intent): LaunchType {
            ...
            return if (intent.isBrowserDeepLink()) {
                ...
                intent.getBooleanExtra(EXTRA_SHOW_FIRST_DECK_CARD, false)
            ) {
                LaunchType.REVIEW
        ...
        /** Opens a card from the first user deck (picker order). */
        fun getReviewFirstDeckIntent(context: Context): Intent =
            Intent(context, IntentHandler::class.java).putExtra(EXTRA_SHOW_FIRST_DECK_CARD, true)
    }
}
